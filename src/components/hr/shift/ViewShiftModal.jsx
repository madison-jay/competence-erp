        ...
        <div className="fixed inset-0 bg-[#000000aa] bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl p-6 relative transform transition-all scale-100 ease-in-out duration-300">
                ...
                <div className="space-y-6">
                    <div>
                        <p className="text-sm font-semibold text-[#000000] uppercase mb-2 ml-2">Shift Type</p>
                        {renderBadge(shift.shiftType)}
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                        <div>
                            <p className="text-sm font-semibold text-[#000000] uppercase">Department</p>
                            <p className="text-lg text-gray-900 font-medium mt-1">{shift.department || 'N/A'}</p>
                        </div>
                        <div>
                            <p className="text-sm font-semibold text-[#000000] uppercase">Date Range</p>
                            <p className="text-lg text-gray-900 font-medium mt-1">
                                {shift.date && shift.endDate ? `${shift.date} - ${shift.endDate}` : 'N/A'}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm font-semibold text-[#000000] uppercase">Start Time</p>
                            <p className="text-lg text-gray-900 font-medium mt-1">
                                {formatTimeWithAmPm(shift.startTime)}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm font-semibold text-[#000000] uppercase">End Time</p>
                            <p className="text-lg text-gray-900 font-medium mt-1">
                ...
                <div className="flex justify-end mt-8">
                    <button
                        type="button"
                        onClick={onClose}
                        className="inline-flex items-center rounded-md border border-transparent bg-[#000000] px-6 py-2 text-sm font-medium text-white shadow-sm hover:bg-[#a67c18] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#000000] focus:ring-offset-2"
                    >
                        Close
                    </button>
                </div>
