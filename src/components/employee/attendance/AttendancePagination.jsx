    ...
    const pageElements = visiblePages.map(page => (
      <button
        ...
        className={`w-8 h-8 flex items-center justify-center rounded-lg font-semibold transition-colors duration-200 
          ${currentPage === page ? 'bg-[#000000] text-white' : 'text-gray-600 hover:bg-gray-200'}`}
      >
        {page}
      </button>
    ));
    ...
    if (startPage > 1) {
      ...
      pageElements.unshift(
        <button
          ...
          className={`w-8 h-8 flex items-center justify-center rounded-lg font-semibold transition-colors duration-200 
            ${currentPage === 1 ? 'bg-[#000000] text-white' : 'text-gray-600 hover:bg-gray-200'}`}
        >
          1
        </button>
      );
    ...
    if (endPage < totalPages) {
      ...
      pageElements.push(
        <button
          ...
          className={`w-8 h-8 flex items-center justify-center rounded-lg font-semibold transition-colors duration-200 
            ${currentPage === totalPages ? 'bg-[#000000] text-white' : 'text-gray-600 hover:bg-gray-200'}`}
        >
          {totalPages}
        </button>
      );
