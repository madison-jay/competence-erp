...
import {
  faBars, faTimes, faThLarge, faPlaneDeparture, faCheckSquare,
  faClipboardList, faMoneyBillAlt, faFileAlt, faArrowRightFromBracket,
  faBoxesStacked, faGraduationCap, faChartBar
...
export default function SideNavBar({
  ...
  const baseNavItems = [
    ...
    { name: "Leaves", icon: faPlaneDeparture, path: "/employee/leave" },
    { name: "Payment History", icon: faMoneyBillAlt, path: "/employee/payroll" },
    { name: "Documents", icon: faFileAlt, path: "/employee/documents" },
    { name: "Knowledge Sharing", icon: faGraduationCap, path: "/employee/kss" },
    { name: "Key Performance", icon: faChartBar, path: "/employee/kpi" },
  ];
  ...
  const navItems = isSalesEmployee
    ? [baseNavItems[0], salesNavItem, ...baseNavItems.slice(1)]
    : baseNavItems;
    ...
    <>
      ...
      >
        <div className={`flex items-center p-4 h-[79.4px] ${isDesktopSidebarExpanded ? 'justify-between' : 'justify-center'} shadow-sm`}>
          {isDesktopSidebarExpanded && (
            <Image src="/competenceng_logo.png" alt="competence logo" width={150} height={20} />
          )}
          <button
            onClick={toggleDesktopSidebar}
            className="p-2 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-[#000000]"
              ...
              <div
                className={`flex items-center py-3 rounded-md mb-2 cursor-pointer
                  ${pathname === item.path
                    ...
                    : 'text-gray-600 hover:bg-[#ffecc0] hover:text-[#000000]'
                  }
